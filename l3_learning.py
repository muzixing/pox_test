...
from pox.lib.revent.pre_arp import lie
from pox.lib.revent.transport import Transport
...
class l3_switch (EventMixin):
    ...
    for k,v in self.lost_buffers.iteritems():
      ...
      for item in list(v):
        ...
        if expires_at < time.time():
          # This packet is old.  Tell this switch to drop it.                 modify this code.
          v.remove(item)
  ...
  def _handle_PacketIn (self, event):
    ...
    if isinstance(packet.next, ipv4):
      ...
      #different swtich has different action
      if  assign_dipd == dpid:
        tran_test = transport.Transport()
        tran_test.deliver_1(event,dpid)
      ...
      if a.prototype == arp.PROTO_TYPE_IP:
        if a.hwtype == arp.HW_TYPE_ETHERNET:
          if a.protosrc != 0:
            #pretend to answer 
            #arp_test=pre_arp.lie()
           # arp_test.cheat(event,dpid)
